# Privacy Policy

**Last updated:** May 3, 2026

---

## 1. Introduction

This Privacy Policy explains how GameLab ("we", "our", or "us") handles information when you use our mobile application.

Our app is designed to provide entertainment through local multiplayer mini-games and does not require account creation.

---

## 2. Information We Collect

We do not collect personal information such as your name, email address, or contact details.

However, third-party services integrated into the app may collect certain data automatically.

---

## 3. Advertising and Third-Party Services

Our app uses Google AdMob to display advertisements.

AdMob may collect and process the following types of data:

* Device identifiers (such as advertising ID)
* IP address
* Device information (model, OS version, language)
* App usage data (interactions with ads)

This data may be used to:

* Provide personalized or non-personalized ads
* Measure ad performance
* Prevent fraud and abuse

For more information, please refer to:
https://policies.google.com/privacy

---

## 4. User Consent (GDPR)

If you are located in the European Economic Area (EEA), we will request your consent before showing personalized ads.

You can:

* Accept personalized ads
* Reject personalized ads (you will see non-personalized ads instead)

You can change your preferences at any time through the app settings.

---

## 5. App Tracking Transparency (iOS)

On iOS devices, we request permission to track your activity across apps and websites as required by Apple.

If you choose not to allow tracking:

* You will still see ads
* Ads may be less relevant (non-personalized)

---

## 6. Data Storage

We do not store personal data on our servers.

Some preferences (such as consent choices) may be stored locally on your device.

---

## 7. Children’s Privacy

Our app is not specifically directed to children under the age of 13.

We do not knowingly collect personal data from children.

---

## 8. Data Sharing

We do not sell, trade, or share your personal data.

Data may be processed by third-party services such as AdMob solely for advertising purposes.

---

## 9. Your Rights

Depending on your location, you may have rights under data protection laws, including:

* The right to access your data
* The right to request deletion
* The right to withdraw consent

Since we do not directly store personal data, requests should be directed to the relevant third-party providers.

---

## 10. Changes to This Policy

We may update this Privacy Policy from time to time.

Any changes will be posted on this page with an updated "Last updated" date.

---

## 11. Contact

If you have any questions, you can contact us at:

**[villalabs.app@gmail.com](mailto:villalabs.app@gmail.com)**

---
